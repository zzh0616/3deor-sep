...
上述 wideband pre-filtering 已随后用正确的 64-frequency exact visibility
operator 完成，而不是用 image cube 或单色结果代替。输入为
`114.7--121.0 MHz` 的 64 频，先在完整带宽上做 `1e-12` hard-DPSS
projection，再截取中央 `116.3--119.4 MHz` 的 32 频估计 PS2D；窄带对照
只在相同中央 32 频上投影。两者共享完整 64 频 operator、1056-source
`Q_beta` context、384 rows/bin 和 16 个固定前景/独立 EoR 相位。

宽带 broad response-localized support 从 `112` 增到 `122` 格，新增 10 格、
无丢失；122 格在所有 16 相位中都满足 20% 单格门。最坏 broad L2 从
`6.351%` 降到 `5.477%`。新增格位于
`k_parallel=0.3027/0.4036 Mpc^-1`，最坏误差为 `5.30%--10.95%`，最坏
前景影响为 `0.162%`。这支持 guard-band 预过滤确实改善低-kpar 滤波边缘，
但仍是无噪声、无 PB、固定 rows 的重叠 windowed-bandpower 结果。

完整记录：
`docs/visibility_qbeta_wideband_20260725_zh.md`；逐格图：
`docs/figures/visibility_qbeta_wideband_grid_20260725.png`。

下一步应冻结 122 格候选 support，构造独立 time/noise splits 并改用 cross
quadratic estimator。窗口选择必须继续只读 response；不能根据 noisy EoR
truth 再删除失败格。
...
- wideband promotion summary：
  `docs/results/visibility_qbeta_wideband_20260725/summary.json`
- wideband broad-support per-cell table：
