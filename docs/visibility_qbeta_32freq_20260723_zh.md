...
### 全部 112 个已计算 output cells 的逐格审计

最终运行实际计算了 112 个 output cells，并且它们全部位于冻结的几何 EoR
window 内。原主结果只保留 20 个，是因为另外 92 个没有满足“至少 80% 的
响应权重位于预声明 40-band reporting region”这一定位门，而不是因为它们
的 `Q_beta` 数值闭合超过 20%。

在不改变任何估计量的逐格事后审计中：

- pure-EoR 的 112/112 cells 均满足绝对误差 `<20%`，最大绝对误差
  `17.8998%`，中位绝对误差 `5.4097%`；
- FG+EoR 的 112/112 cells 也均满足绝对误差 `<20%`，最大绝对误差
  `17.8945%`，中位绝对误差 `5.4119%`；
- 两套误差逐格近乎相同，因此这 112 格的主误差来自有限 response
  calibration/probe，而不是 residual foreground；
- 102/112 个响应窗有至少 95% 的权重留在完整几何 EoR window 内；最高
  `k_parallel` 层的 10 格只有 `74.21--78.91%`，主要和 radial-Nyquist
  context 混合。

但这些结果仍不能表述为“恢复了 112 个局域二维格点”。每个 output 对应
`WP` 而不是 delta-function band：same-source-cell response fraction
只有 `0.188--0.522`、中位数 `0.415`，window effective width 为
`2.962--6.370` 个 source bands、中位数 `3.853`。而且“95% 权重留在完整
EoR window”不是原先预声明的 promotion gate。它只能作为下一轮冻结新 support
并用独立 probes/sky realization 验证的候选，不能事后替换原 20-window 主结果。

下图在完整 32x16 网格上给出 112 格的 signed error；浅绿色为空间上属于
EoR window 但本轮未计算的 cells，金色边框仍只表示原先冻结的 20 个窗口。
全部 512 格的几何状态、响应定位量、pure/total 误差和前景比值见
`docs/results/visibility_qbeta_cell_errors_20260724.csv`。

![完整二维网格逐格恢复误差](figures/visibility_qbeta_full_grid_cell_errors.png)
...
在相同 exact operator、rows、544-source context 和无噪声口径下，新增
Hann-only、soft-DPSS、DPSS cutoff、Blackman-Harris、三阶 Chebyshev 和
DPSS-rank-matched Chebyshev 对照。Hann-only 和三阶 Chebyshev 明确失败；
Blackman-Harris 扩宽 window 且不改善前景；soft-DPSS 与更松 cutoff 在原
20 格仍安全，但其新增低-`k_parallel` support 多数被 residual foreground
污染。

`1e-10` hard-DPSS 是唯一有小幅扩展价值的 DPSS 候选：比默认 `1e-12`
增加 3 个 response-localized cells，16 个固定前景/独立 EoR phases 中仍
保持 20% 内。等秩 Chebyshev 增加 2 格，但它用 11--14 个方向且与 DPSS
子空间的最小 canonical correlation 为 `0.972--0.993`，应解释为近等价
基底，而不是低阶多项式成功。完整方法、逐项结果和文献边界见
[`visibility_qbeta_filter_ablation_20260724_zh.md`](visibility_qbeta_filter_ablation_20260724_zh.md)。
