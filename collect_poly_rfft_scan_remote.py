#!/usr/bin/env python3
"""
Collect and merge poly/rfft scan results from remote workers.

We avoid rsync'ing FITS outputs back (they are huge). Instead we fetch the small
CSV/MD artifacts over SSH and merge locally.
"""
...
from run_poly_rfft_scan import _candidate_summary, _write_markdown
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Collect poly/rfft scan outputs from remote workers.")
    p.add_argument(
        "--dispatch-manifest",
        type=Path,
        required=True,
        help="Local dispatch manifest produced by dispatch_poly_rfft_remote.py.",
    )
    p.add_argument(
        "--output-root",
        type=Path,
...
def main() -> int:
    ...
    for rec in payload.get("launches", []):
        ...
        for filename in (
            "poly_rfft_scan_results.csv",
            "poly_rfft_scan_rank.csv",
            "poly_rfft_scan_summary.md",
            "manifest.json",
        ):
    ...
    for rec in payload.get("launches", []):
        ...
        break
    _write_markdown(merged_root / "poly_rfft_scan_summary_merged.md", ranked, baseline_fixed)
