#!/usr/bin/env python3
"""
Collect and merge lagcorr-envelope scan results from remote workers.

We intentionally avoid rsync-ing the full run directories (FITS outputs can be large).
...
from run_lagcorr_envelope_scan import _candidate_summary, _write_markdown
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect lagcorr envelope scan outputs from remote workers.")
    parser.add_argument(
        "--dispatch-manifest",
        type=Path,
        required=True,
        help="Local dispatch manifest produced by dispatch_lagcorr_envelope_remote.py.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=None,
        help="Local output root (default: <manifest_parent>/<remote_run_root_name>/).",
    )
...
def main() -> int:
    ...
    for rec in payload.get("launches", []):
        ...
        for filename in (
            "lagcorr_env_results.csv",
            "lagcorr_env_rank.csv",
            "lagcorr_env_summary.md",
            "manifest.json",
            "worker.log",
        ):
        ...
        results_path = local_worker_dir / "lagcorr_env_results.csv"
        if results_path.exists():
            for row in _read_csv_rows(results_path):
                row["_source"] = str(results_path)
                merged_rows.append(row)

    partial_path = merged_root / "lagcorr_env_results_partial_merged.csv"
    _write_csv(partial_path, merged_rows)

    merged_path = merged_root / "lagcorr_env_results_merged.csv"
    _write_csv(merged_path, merged_rows)

    ranked = _candidate_summary(merged_rows, exclude_datasets=exclude_names)
    rank_path = merged_root / "lagcorr_env_rank_merged.csv"
    ...
    _write_markdown(merged_root / "lagcorr_env_summary_merged.md", ranked, baseline_fixed)
