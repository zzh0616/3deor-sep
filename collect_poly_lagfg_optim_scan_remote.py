#!/usr/bin/env python3
"""
Collect and merge poly+lag_fg_corr+optimizer scan results from remote workers.

We avoid fetching FITS outputs back (huge). Instead we fetch small CSV/MD/JSON
artifacts over SSH and merge locally.
"""
...
from run_poly_lagfg_optim_scan import _candidate_summary, _write_markdown
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Collect poly+lag_fg_corr+optimizer scan outputs from remote workers.")
    p.add_argument(
        "--dispatch-manifest",
        type=Path,
        required=True,
        help="Local dispatch manifest produced by dispatch_poly_lagfg_optim_scan_remote.py.",
    )
    p.add_argument(
        "--output-root",
        type=Path,
        default=None,
        help="Local output root (default: sibling dir named like the remote run root under runs/remote/<date>/).",
    )
    p.add_argument("--overwrite", action="store_true", help="Overwrite previously collected files if they exist.")
    return p.parse_args()
...
def main() -> int:
    ...
    for rec in payload.get("launches", []):
        ...
        results_path = local_worker_dir / "poly_lagfg_optim_scan_results.csv"
        if results_path.exists():
            for row in _read_csv_rows(results_path):
                row["_source"] = str(results_path)
                merged_rows.append(row)

    merged_path = merged_root / "poly_lagfg_optim_scan_results_merged.csv"
    _write_csv(merged_path, merged_rows)

    ranked = _candidate_summary(merged_rows, exclude_datasets=exclude_names)
    rank_path = merged_root / "poly_lagfg_optim_scan_rank_merged.csv"
    ...
    for rec in payload.get("launches", []):
        ...
        try:
            ...
            continue
        meta = worker_manifest
        break
    _write_markdown(merged_root / "poly_lagfg_optim_scan_summary_merged.md", ranked, meta)
